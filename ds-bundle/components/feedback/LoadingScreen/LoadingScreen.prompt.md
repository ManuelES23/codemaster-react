LoadingScreen from codemaster-react. Use via `window.CodeMaster.LoadingScreen` (bundle loaded from the root `_ds_bundle.js`). Wrap the tree in `<PreviewRouter>` (full provider chain in README.md — components read theme/i18n from that context).
